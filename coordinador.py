...
class Inicio:
    def __init__(self):
        self.ventana = tk.Tk()
        self.ventana.title("Perfil coordinador de producción")
        ...
        self.conn = sqlite3.connect('coordinador.db')
        self.cursor = self.conn.cursor()

        # Crear la tabla de usuarios si no existe
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS coordinador (
    ...
    def iniciar_sesion(self):
        ...
        if usuario and password:
            self.cursor.execute("SELECT * FROM coordinador WHERE usuario = ? AND contraseña = ?", (usuario, password))
            if self.cursor.fetchone() is not None:
                try:
                    # Ejecuta "ui_menu.py" en la misma ubicación que este archivo
                    subprocess.Popen(["python", "tarea_coordinador.py"])
    ...
    def crear_usuarios_predefinidos(self):
        ...
        for usuario, contraseña in usuarios_predefinidos:
            try:
                self.cursor.execute("INSERT INTO coordinador (usuario, contraseña) VALUES (?, ?)", (usuario, contraseña))
                self.conn.commit()
